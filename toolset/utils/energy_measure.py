...
class energy : 
    ...
    def __init__(self,pkg0,pkg1,time): 
        self.pkg0 = pkg0
        self.pkg1 = pkg1
        self.time = time 
    ...
    def __add__(self,x): 
        if x == 0 : 
            return self 
        return energy(self.pkg0+x.pkg0,self.pkg1+x.pkg1,self.time + x.time)
    ...
    def __str__(self):
        return '{:.2f}s, {:.2f} J, {:.2f} J'.format(self.time,float(self.pkg0)/10**6,float(self.pkg1)/10**6)       